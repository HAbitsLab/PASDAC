...
def metricsEvaluation(labels, pred, target_class=None):
    ...
    if target_class == None:
        print('No target class is assigned.\n')        
        label_uniq = np.unique(labels)

        for tar_class in label_uniq:
            print('For class',tar_class)
            ...
            print('  fallout is', fallout)
            print('  specificity is', specificity)
            print('  NPV is', NPV)
            print('  FDR is', FDR)
            print('  FNR is', FNR)
            print('  accuracy is', accuracy)
            print('  f1_pos is', f1_pos)
            print('  MCC is', MCC)
            print('  CKappa is', CKappa, '\n')
    else:
        print('Target class is Class',target_class)
...
if __name__ == "__main__":
    ...
    cm, precision, recall, fallout, specificity, NPV, FDR, FNR, accuracy, f1_pos, MCC, CKappa = metricsEvaluation(labels, pred,1)
    print('  confusion matrix is', cm)
    print('  precision is', precision)
    print('  recall is', recall)
    print('  fallout is', fallout)
    ...
    print(metricsEvaluation(labels, pred))
